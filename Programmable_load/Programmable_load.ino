...
int brightsw=0;   //
boolean yes = true;    //to check if load on or off/ switch is pressed or not
...
{ attachInterrupt(digitalPinToInterrupt(pinA), isr,LOW); // to get values from encoder
 ...
 //lcd.setCursor(0,0);                                   
 //lcd.print("I Feel The Need");                    // this for fun if you want this too then uncomment it
 //lcd.write(byte(0));
 //lcd.setCursor(0,1);
 ...
 for(int i=0;i<3;i++)                         // loop to give graphics in which man is waving hands up and down
 {delay(50);                                 // wait
...
//--------------------cursor position is change(0to50)------------------------------------------------
void encoderdisplay()
{
...
void lcd_display()
{ lcd.setCursor(0,0);
  if(amps>1.52&& amps<1.65)                                             //completely ignore this part because of my current sensor wire get soo hot
  {amps=1.99;}                                                         //i will improve current sensor
  else if(amps>2.01&& amps<2.30)
  {amps=3.07;} else if(amps>=2.40&&amps<=2.52)
                  {amps=4.13;} 
